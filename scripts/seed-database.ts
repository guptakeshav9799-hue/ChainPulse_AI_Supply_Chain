...
async function seedDatabase() {
  ...
  try {
    ...
    const { data: shipments, error: shipmentsError } = await supabase
      .from('shipments')
      .insert(seedShipments)
      .select()

    if (shipmentsError) {
      console.error('Error inserting shipments:', shipmentsError)
    } else {
      console.log(`✅ Successfully inserted ${shipments?.length || 0} shipments`)
    }

    console.log('\n⚠️  Inserting disruptions...')
    const { data: disruptions, error: disruptionsError } = await supabase
      .from('disruptions')
      .insert(seedDisruptions)
      .select()

    if (disruptionsError) {
      console.error('Error inserting disruptions:', disruptionsError)
    } else {
      console.log(`✅ Successfully inserted ${disruptions?.length || 0} disruptions`)
    }
